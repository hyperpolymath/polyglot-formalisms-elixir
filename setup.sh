...
# setup.sh — Universal setup script for polyglot-formalisms-elixir
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/polyglot-formalisms-elixir/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== polyglot-formalisms-elixir Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/polyglot-formalisms-elixir.git"
        info "Then: cd polyglot-formalisms-elixir && ./setup.sh"
        exit 1
    fi
